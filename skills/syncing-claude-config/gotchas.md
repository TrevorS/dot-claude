...
- Accepted is not read. The audit checks theme keys against the built-in palettes, which is what the loader accepts, not what the UI reads. In 2.1.282, `fastModeShimmer`, `inactiveShimmer`, `permissionShimmer`, `promptBorderShimmer` and `warningShimmer` sit in all six palettes with zero reads outside them, and no code builds the names by appending "Shimmer" (the one other hit per key is the palette's bytecode identifier table). Keep them: removing them turns every run into a `missing` report, and they would apply again if a later build reads them. To find these, count each key's quoted and `.key` references outside the `{key:"` palette definitions, using `bytes.find`, not a regex that starts with `.{0,N}`, which takes minutes on the binary.
