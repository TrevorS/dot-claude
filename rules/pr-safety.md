...
Force-pushing is fine only when Teej confirms or the PR has zero review activity. `hooks/git_dangerous_flags.sh` blocks the git forms (`push --force`, `commit --amend`, `reset --hard`, `--no-verify`, `gh pr merge --admin`) for the agent; when you and Teej agree it is the right move, hand him the command to run with the `!` prefix. No hook covers the jj path: `jj git push` after a `jj squash`, `jj rebase`, or `--ignore-immutable` rewrite force-pushes silently, so the check above is the only gate there.
