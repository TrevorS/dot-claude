...
`Edit`, `Write`, `NotebookEdit`, `WebFetch`, `WebSearch`, `Skill`, `ToolSearch`,
...
Async: `FileChanged`, `CwdChanged`, `DirectoryAdded`, `ConfigChange`, `InstructionsLoaded`, `WorktreeCreate`, `WorktreeRemove`, `Notification`, `SubagentStart`, `SubagentStop`, `TaskCreated`, `TaskCompleted`, `PreCompact`, `PostCompact`, `TeammateIdle`, `Elicitation`, `ElicitationResult`, `MessageDisplay`, `PreModelSwitch`, `PostModelSwitch`

That is all 33 documented events (verified against the hooks reference, 2026-09-18; `PreModelSwitch`/`PostModelSwitch` landed in 2.1.251).
...
Additional output fields: `updatedToolOutput` (replace tool output, all tools), `terminalSequence` (emit OSC sequences for desktop notifications), `sessionTitle` (set session title, SessionStart only), `reloadSkills: true` (re-scan skill dirs, SessionStart only). `duration_ms` is in the `PostToolUse` and `PostToolUseFailure` input (2.1.119).
...
`background` applies only with `context: fork` (default `true` since 2.1.218) — set `false` to wait for the result in the invoking turn. For backgrounded fork skills, `disallowed-tools: AskUserQuestion` stops a question from parking as a needs-input request.
...
`.claude/rules/*.md` files. Each can have `paths:` frontmatter (`paths: ["**/*.py"]`) to load only when matching files are touched. Smaller, narrower files load less context per session.
...
3. Test each hook script standalone with synthetic stdin before wiring
